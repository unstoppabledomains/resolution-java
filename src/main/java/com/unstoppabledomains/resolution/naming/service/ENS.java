...
import com.unstoppabledomains.resolution.contracts.ens.NameWrapper;
...
public class ENS extends BaseNamingService {
    ...
    private final NameWrapper nameWrapperContract;
    private final JsonElement ensConfigJsonElement;
        ...
        final JsonReader jsonReader =
            new JsonReader(
                new InputStreamReader(
                    BaseContract.class.getResourceAsStream("ens/config/ens_config.json")
                )
            );
        this.ensConfigJsonElement = new Gson().fromJson(jsonReader, JsonElement.class);
        this.registryContract = (Registry) buildContract(
            config.getContractAddress(),
            EnsContractType.Registry, 
            provider
        );
        this.nameWrapperContract = (NameWrapper) buildContract(
            this.getContractAddressFromConfig(EnsContractType.NameWrapper),
            EnsContractType.NameWrapper,
            provider
        );
    }
    ...
    public String getTokenUri(BigInteger tokenID) throws NamingServiceException {
        Boolean isWrappedDomain = nameWrapperContract.isWrappedDomain(tokenID);
        if (isWrappedDomain) {
            return "https://metadata.ens.domains/" +  
            this.getNetwork().getName() + "/" + 
            this.getContractAddressFromConfig(EnsContractType.NameWrapper) + "/" + 
            tokenID.toString();
        }
        ...
        return "https://metadata.ens.domains/" + 
            this.getNetwork().getName() + "/" + 
            this.getContractAddressFromConfig(EnsContractType.BaseRegistrarImplementation) + "/" + 
            tokenID.toString();
    }
    ...
    public String getAddress(String domain, String network, String token) throws NamingServiceException {
        ...
        //     new NSExceptionParams("c", token.toUpperCase()));
        // }
        // Resolver resolver = getResolverContract(domain);
        // byte[] tokenId = tokenId(domain);
        // return resolver.addr(tokenId, token.toUpperCase());
    }
    ...
    //         new NSExceptionParams("m|n|a", "getReverseTokenId", "ENS", address));
    //     }
    ...
    private BaseContract buildContract(String address, EnsContractType type, IProvider provider) {
        switch (type) {
            ...
            case NameWrapper:
                return new NameWrapper(blockchainProviderUrl, address, provider);
            default:
                // Return registry contract as default.
    ...
    private String getContractAddressFromConfig(EnsContractType contract) {
        return ensConfigJsonElement.getAsJsonObject()
            .get("networks").getAsJsonObject()
            .get(Integer.toString(this.getNetwork().getCode())).getAsJsonObject()
            .get("contracts").getAsJsonObject()
            .get(contract.toString()).getAsJsonObject()
            .get("address").getAsString();
    }

    // private Boolean determineIsWrappedDomain(BigInteger tokenId) {
